...
# =============================================================================
        ...
        if self.enable_dueling_network :
            self.target_q_values_func = K.function([self.target_network.layers[0].input], [self.target_network.layers[6].output])
        else:
    ...
    def update_predict_network(self):
        ...
        if self.enable_double_dqn:
            actions = np.argmax(self.calc_q_values(preprocessed_new_states), axis=1)
            # Now, estimate Q values using the target network but select the values with the
            # highest Q value wrt to the online model (as computed above).
            max_q_values = q_values[range(self.batch_size), actions]
        else:
            # get the max q_values
            max_q_values = np.max(q_values,axis = 1)
        max_q_values[is_terminals] = 0.0
        targets = rewards + self.gamma * max_q_values # this will learn the error as well
                ...
                if self.num_steps > self.num_burn_in:
                    ...
                    if self.num_steps % self.train_freq == 0:
                        ...
                        if num_updates % 10000 == 0:
                            ...
                            if self.enable_double_dqn:
                                self.q_network.save_weights('%s/double_dqn_model_weights_%d.h5' % (self.save_path, num_updates //10000))
                            if self.enable_double_dqn:
                                self.q_network.save_weights('%s/dueling_dqn_model_weights_%d.h5' % (self.save_path, num_updates //10000))
                if is_terminal or (max_episode_length is not None and t > max_episode_length):
                    break
                state = next_state
                rewards_set.append(total_reward)
